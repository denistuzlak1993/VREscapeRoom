	...
	public class GameClient : TNEvents
	{
		...
		public void JoinChannel (int channelID, string levelName, bool persistent, int playerLimit, string password)
		{
			...
			if (isConnected && !IsInChannel(channelID) && !mJoining.Contains(channelID))
			{
				...
				#if UNITY_EDITOR
				Debug.Log($"From Join channel TMGameClient Level name is {levelName}");
				#endif
		...
		{
			#if UNITY_EDITOR
			//Debug.Log($"TN Game client LoadLevel() level name is {levelName}");
			#endif
#if !MODDING
			if (isConnected && IsInChannel(channelID))
			{
		...
		bool ProcessPacket (Buffer buffer, IPEndPoint ip)
		{
			...
			switch (response)
			{
				...
				case Packet.ResponseLoadLevel:
				{
					...
					int channelID = reader.ReadInt32();
					string scene = reader.ReadString();// levelNameSwitch;// reader.ReadString();
					#if UNITY_EDITOR
					Debug.Log("Scene string from response is " +  scene);
					#endif
