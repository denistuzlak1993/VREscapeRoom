...
namespace TNet
{
	...
	public class TNManager : MonoBehaviour
	{
		...
		static public LoadSceneFunc onLoadScene = delegate (string levelName)
		{
			...
			{
				Debug.Log($@"Loading level with name: {levelName}");
		...
		void SetDefaultCallbacks ()
		{
			...
			mClient.onLoadLevel = delegate (int channelID, string levelName)
			{
				...
				if (!string.IsNullOrEmpty(levelName))
				{
					Debug.Log($"Id from TN manager before LoadLevelCoroutine is {channelID} and level name is {levelName}");
		...
		{
			yield return null;
			Debug.Log($"keyPairValue is {pair.Value.ToString()}");
